...
| 6.5 | 功能完整性开发 | 📎 规划中 | 速率限制、文件上传、消息增强 |
...
### 阶段 6.5：功能完整性开发（额外功能补全）📎

**目标**：补充核心聊天功能之外的附加功能，提升产品完整性。
...
- [ ] **6.5.1 速率限制中间件**
  - 实现基于 IP 的请求速率限制
  - 实现基于用户的请求速率限制
  - 针对不同接口设置不同限制策略（认证/消息/房间）
  - 返回 429 Too Many Requests 状态码

- [ ] **6.5.2 文件上传与资源管理**
  - 实现文件上传接口 `POST /api/v1/upload`
  - 支持文件类型分类（图片/文档/视频/其他）
  - 文件存储架构：
    - 本地存储：`uploads/{category}/{year}/{month}/{uuid}.{ext}`
    - 可选：云存储集成（S3/OSS）
  - 文件元数据管理（文件名、大小、MIME、哈希）
  - 文件访问权限控制
  - 图片消息和文件消息的 WebSocket 支持
...
#### 技术要点
- 使用 `multer` 或 `axum` 的 multipart 支持处理文件上传
- 使用 `sha2` 计算文件哈希实现去重
- 使用 `mime` 进行文件类型检测
- 文件大小限制和类型白名单
- 异步文件存储操作
