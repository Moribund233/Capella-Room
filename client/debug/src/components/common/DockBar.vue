<template>
  <div v-if="visible" ref="dockRef" class="dock-bar" :class="[`position-${position}`, { 'is-expanded': isExpanded }]"
    ...
    <div class="dock-panel">
      <!-- Dock 项目列表 -->
      <div class="dock-items" :class="{ 'is-vertical': isVertical }">
        <div v-for="(item, index) in items" :key="item.key" class="dock-item-wrapper"
          :style="{ animationDelay: `${index * 50}ms` }">
          <n-tooltip placement="top" trigger="hover" :show-arrow="false">
            <template #trigger>
              <button class="dock-item" :class="{ 'is-active': activeKey === item.key }"
                @click.stop="handleItemClick(item)">
                <div class="item-icon-wrapper">
                  <component :is="item.icon" class="item-icon" :size="22" />
                </div>
                <!-- 激活指示器 -->
...
.dock-item:hover .item-icon-wrapper {
  transform: scale(1.1);
}
...
.dock-item.is-active .item-indicator {
