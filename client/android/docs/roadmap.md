...
- [x] 长按消息弹出操作菜单（编辑/删除）
- [x] 消息编辑 UI（输入框预填充原内容）
- [x] 调用 `PUT /api/v1/messages/:id` 编辑消息
- [x] WebSocket `EditMessage` / `MessageEdited` 实时同步
- [x] 本地数据库更新编辑状态和编辑历史标记
- [x] 消息删除确认对话框
- [x] 调用 `DELETE /api/v1/messages/:id` 删除消息
- [x] WebSocket `DeleteMessage` / `MessageDeleted` 实时同步
- [x] 软删除消息 UI 展示（"此消息已删除"）
- [x] 仅允许编辑/删除自己发送的消息

#### Phase 6.2: 消息搜索 ✅
**2026-06-05**
- [x] 聊天页顶部搜索栏 UI（底部弹窗形式）
- [x] 本地消息缓存搜索（Room 数据库）
- [x] 远程 API 搜索 `GET /api/v1/messages/search`
- [x] 搜索结果高亮显示
- [x] 防抖搜索（300ms）
- [ ] 点击搜索结果跳转至消息位置（待实现）
- [ ] 搜索历史记录（待实现）
...
#### Phase 6.3: 消息已读回执 ✅
**2026-06-05**
- [x] 数据库添加 `is_read` / `read_count` / `read_by` 字段
- [x] 数据库迁移 v1 -> v2
- [x] WebSocket `MessageRead` / `MessageReadReceipt` 处理
- [x] 本地已读状态更新逻辑
- [ ] 消息已读 UI 标记（双勾图标）
- [ ] 发送已读确认（滚动到消息时触发）
- [ ] 显示已读人数（群聊）
...
**2026-06-05**
- [x] 房间详情页邀请按钮
- [x] 创建邀请码 `POST /api/v1/rooms/:id/invitations`
- [x] 邀请码有效期和次数限制设置
- [x] 邀请列表管理（查看/撤销）
- [x] 分享邀请码（系统分享 Sheet）
- [x] 通过邀请码加入 `POST /api/v1/rooms/join-by-invite`
- [x] 邀请码输入对话框
- [x] 验证邀请码有效性 `GET /api/v1/rooms/validate-invite`

#### Phase 6.5: 用户搜索 ✅
**2026-06-05**
- [x] 频道列表页用户搜索入口
- [x] 用户搜索 UI（搜索栏 + 结果列表）
- [x] 调用 `GET /api/v1/users/search` 搜索用户
- [x] 搜索结果展示（头像/用户名/在线状态）
- [x] 点击用户发起私聊 `POST /api/v1/rooms/direct`
- [x] 搜索历史记录

#### Phase 6.6: 文件/图片上传和预览 ✅
**2026-06-05**
- [x] 附件按钮点击打开文件选择器
- [x] 图片选择（单张/多张）
- [x] 文件上传 `POST /api/v1/upload` / `POST /api/v1/upload/image`
- [x] 附件类型选择菜单（图片/文件）
- [ ] 上传进度显示（待实现）
- [ ] WebSocket `FileUploadComplete` 接收完成通知（待实现）
- [ ] 图片消息气泡（带圆角和点击预览）（待实现）
- [ ] 文件消息气泡（文件名/大小/下载按钮）（待实现）
- [ ] 图片预览（全屏查看、缩放、保存）（待实现）
...
#### Phase 6.8: 消息编辑历史（可选）
- [ ] 已编辑消息标记（"已编辑"标签）
- [ ] 点击查看编辑历史 `GET /api/v1/messages/:id/history`
- [ ] 编辑历史弹窗/页面
